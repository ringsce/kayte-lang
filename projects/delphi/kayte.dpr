...
uses
  ...
  Winapi.ShellAPI,
  {$ENDIF }
  cli in '..\..\dsource\cli.pas',
  Bytecode in '..\..\dsource\Bytecode.pas',
  TestBytecode in '..\..\dsource\TestBytecode.pas',
  VirtualMachine '..\..\dsource\VirtualMachine.pas',
  XMLParser in '..\..\dsource\XMLParser.pas',
  sdk in '..\..\dsource\sdk.pas',
  c99 in '..\..\dsource\c99.pas',
  kayte2pce in '..\..\dsource\kayte2pce.pas',
  KayteLibLoader in '..\..\dsource\KayteLibLoader.pas',
  basic in '..\..\dsource\basic.pas';
...
procedure DownloadFromRepo(const RepoURL: string);
begin
  {$IFDEF MSWINDOWS}
  ShellExecute(0, 'open', 'curl.exe', PChar('-LOk ' + RepoURL), nil, SW_SHOWNORMAL);
  {$ELSE}
  ShellExecute(0, 'open', '/usr/bin/curl', PChar('-LOk ' + RepoURL), nil, SW_SHOWNORMAL);
  {$ENDIF}
end;
...
begin
  ...
  try
    ...
    for I := 0 to High(Headers) do
      if SameText(Headers[I].Name, 'Last-Modified') then
        Writeln('Last-Modified: ', Headers[I].Value);
  except
    on E: Exception do
      Writeln('Error checking for updates: ', E.Message);
  end;
  Client.Free;
end;
