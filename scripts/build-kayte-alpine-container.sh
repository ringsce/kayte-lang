...
# for GUI projects like projects/vb6interpreter.lpi.
#
# Usage:
...
IMAGE_NAME="${IMAGE_NAME:-kayte-alpine-builder}"
CONTAINER_NAME="kayte-alpine-build"

IMAGE_CTX_DIR="$REPO_ROOT/build/alpine/image"
OUT_DIR="$REPO_ROOT/build/alpine"
...
compile_kayte() {
    ...
    container run --rm \
        ...
        -lc "rm -f lib/libkaytearm64elf.so \
            ...
            && cp -f kayte /workspace/build/alpine/kayte \
            && cp -f /workspace/lib/libkaytearm64elf.so /workspace/build/alpine/"
...
compile_vb6interpreter() {
    ...
    container run --rm \
        ...
        -lc "lazbuild --lazarusdir=/opt/lazarus vb6interpreter.lpi \
            && cp -f vb6interpreter /workspace/build/alpine/vb6interpreter-linux-arm64"
